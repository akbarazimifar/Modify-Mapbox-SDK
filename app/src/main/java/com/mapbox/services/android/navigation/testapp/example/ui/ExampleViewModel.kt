...
class ExampleViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var routeFinder: RouteFinder? = null

    init {
        if (retrieveProfileFromPreferences() != null && retrieveOfflineVersionFromPreferences() != null) {
        ...
        locationEngine = LocationEngineProvider.getBestLocationEngine(getApplication<Application>().applicationContext)
        locationEngineCallback = ExampleLocationEngineCallback(location)

        // Initialize navigation and pass the LocationEngine
        navigation = MapboxNavigation(getApplication<Application>().applicationContext, accessToken)
    ...
    internal fun updateProfile() {
        retrieveProfileFromPreferences()?.let { routeFinder?.updateProfile(it) }
    }
    ...
    private fun normalizeForTraffic(string: String?): String? {
        ...
        return if (string.equals("driving")) {
            "driving-traffic"
        } else {
            normalizedString
        }
